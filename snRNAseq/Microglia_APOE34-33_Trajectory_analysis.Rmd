...
The code in this script was used to perform a trajectory analysis and produce Figure 1 D-E
...
## 0.2 Set up color palette
### 0.2.1 All UMAP colors
```{r}
################################################################################
### Setup colors 
################################################################################
...
umap.colors.lf <- c("Cluster1" =  colors[1],"Cluster3" =  colors[2],"Cluster8" = colors[3],"Cluster10" = colors[4],"Cluster4" = colors[5],
                 "Cluster9"  = colors[6],"Cluster2"  =colors[7],"Cluster7" = colors[8],"Cluster5"= colors[9], "Cluster6" = colors[10],
                 "Cluster11" = colors[11], "Cluster12" =colors[12])
umap.colors2 <- umap.colors[order(as.integer(names(umap.colors)))]
umap.colors2 <- umap.colors.lf[order(as.integer(names(umap.colors.lf)))]

################################################################################
### Setup colors when only looking at Cluster 6 and 8
################################################################################

umap.colors <- c("1" =  "grey80","3" =  "grey80","8" = colors[3],"10" = "grey80","4" = "grey80",
                 "9"  = "grey80","2"  ="grey80","7" = "grey80","5"= "grey80", "6" = colors[10],
                 "11" = "grey80", "12" ="grey80")

umap.colors.lf <- c("Cluster1" =  "grey80","Cluster3" = "grey80","Cluster8" = colors[3],"Cluster10" = "grey80","Cluster4" = "grey80",
                 "Cluster9"  = "grey80","Cluster2"  ="grey80","Cluster7" = "grey80","Cluster5"= "grey80", "Cluster6" = colors[10],
                 "Cluster11" = "grey80", "Cluster12" ="grey80")


umap.colors2.p <- umap.colors[order(as.integer(names(umap.colors)))]
umap.colors2.p <- umap.colors.lf[order(as.integer(names(umap.colors.lf)))]
...
```{r}
# Load Data
Microglia <- readRDS(paste0("~/6.2 MicrogliaFinalresubclustered_updated-ClusterNames.rds"))
...
## 2.1 Produce Figure 1.C
```{r}
# Produce the Trajectory Plot
pdf(file = paste0('~/MG_APOE34-33_TrajectoryAnalysis.pdf'))
print(CellDimPlot(Microglia.slingshot, group.by = 'reordered_mappings', reduction = "umap", 
      ...
      title = "", legend.position="none", bg_color="white", theme_use = "theme_classic", raster=FALSE, edge_color = "white", lineages_palcolor = lineages_colors_end_darkest))
dev.off()
```

## 3.0 Produce Figure 1.D
```{r}
...
for (entry in gene_list){
    ...
    ggsave(
    paste0('~/plots/dynamic plots/Compare_Lineages123/MG_APOE34-33_AllClusters_Compare_Lineages123_', 
    entry,
    '.pdf'),
