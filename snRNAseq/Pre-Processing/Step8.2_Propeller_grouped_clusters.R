...
# Test these parameters in grouped clusters
# Date: 19/02/2024
...
######### Diagnosis ############   
propeller_result <- propeller(clusters=Microglia_subset@meta.data$grouped_clusters, sample=Microglia_subset@meta.data$sample, group=Microglia_subset@meta.data$diagnosis)
fwrite(propeller_result , paste0(Output.dir.subset, "Propeller_results/Diagnosis.csv"), row.names = TRUE)
######### Batch ############      
...
fwrite(propeller_result , paste0(Output.dir.subset, "Propeller_results/Batch.csv"), row.names = TRUE)
######### APOE ##############     
...
fwrite(propeller_result , paste0(Output.dir.subset, "Propeller_results/APOE_genotype.csv"), row.names = TRUE)
######### Sex ##############      
propeller_result <- propeller(clusters=Microglia_subset@meta.data$grouped_clusters, sample=Microglia_subset@meta.data$sample, group=Microglia_subset@meta.data$sex)
fwrite(propeller_result , paste0(Output.dir.subset, "Propeller_results/Sex.csv"), row.names = TRUE)
######### Group ##############      
propeller_result <- propeller(clusters=Microglia_female_AD@meta.data$grouped_clusters, sample=Microglia_female_AD@meta.data$sample, group=Microglia_female_AD@meta.data$group)
fwrite(propeller_result , paste0(Output.dir.subset, "Propeller_results/Group.csv"), row.names = TRUE)
...
fwrite(props$Proportions , paste0(Output.dir.subset, "Propeller_results/Cluster_proportions_whole_dataset_by_group.csv"), row.names = TRUE)
...
fwrite(props$Proportions , paste0(Output.dir.subset, "Propeller_results/Cluster_proportions_whole_dataset.csv"), row.names = TRUE)

# Plot proportions bar plot per sample
pdf(file = paste0(Output.dir.subset, "Propeller_results/Cell_type_proportions_bar_plot.pdf"))
barplot(props$Proportions, col = c("orange","purple", "darkgreen", "red", "lightblue", "pink", "yellow", "grey", "lightgreen", "brown", "darkblue", "white", "gold"), 
...
fwrite(eBayes_result , paste0(Output.dir.subset, "Propeller_results/Post-mortem delay.csv"), row.names = TRUE)
######### age ################    
...
fwrite(eBayes_result , paste0(Output.dir.subset, "Propeller_results/Age.csv"), row.names = TRUE)
...
fwrite(eBayes_result , paste0(Output.dir.subset, "Propeller_results/Tau.csv"), row.names = TRUE)
...
fwrite(model, paste0(Output.dir.subset, "Propeller_results/Propeller_final_diagnosis_effect.csv"), row.names = TRUE)
...
model <- propeller.anova(prop.logit,design = design.anova, coef=c(1,2), robust=TRUE, 
                         trend = FALSE, sort=TRUE)
fwrite(model, paste0(Output.dir.subset, "Propeller_results/Propeller_final_APOE_effect.csv"), row.names = TRUE)
